...
int main() {
    ...
    */
    ...
    /*
    Шаблонным называется класс - тип полей которого определяются
    при создании объекта,для того чтобы сделать класс шаблонным,
    перед ним нужно создать шаблон.

    После создания шаблона после этого абсолютно любая переменная
    член-класса может быть шаблонного типа.
    Все методы класса автоматически становятся шаблонными.

    От ветки 'master' создать ветку 'templated', и в ней шаблонизировать
    сначала односвязный список, а потому двусвязный список.
